...
def _get_samples(spacy_dir: str, language_model_name: str) -> Dict[str, List] | None:
    ...
    if spacy_files:
        ...
        for file_path in spacy_files:
            temp_positive_samples, temp_negative_samples = _get_file_samples(
                file_path, nlp.vocab
            )
...
def _split_samples(
    samples, train_pct: float, val_pct: float
) -> Tuple[List, List, List]:
    train_samples = samples[0 : int(len(samples) * train_pct)]
    val_samples = samples[
        len(train_samples) : int(len(samples) * (train_pct + val_pct))
    ]
...
def _interleave_proportional(list1, list2):
    ...
    insert_indices = [
        round(i * (len_long + len_short) / len_short) for i in range(len_short)
    ]
...
def create_dataset(
    ...
    positive_samples, negative_samples = _get_samples(
        spacy_dir_path, language_model_name
    )
    logging.info(f"Total positive samples: {len(positive_samples)}")
    logging.info(f"Total negative samples: {len(negative_samples)}")
    train_pos_samples, val_pos_samples, test_pos_samples = _split_samples(
        positive_samples, train_pct, val_pct
    )
    train_neg_samples, val_neg_samples, test_neg_samples = _split_samples(
        negative_samples, train_pct, val_pct
    )
...
def _save_to_disk(
    samples: List[Doc], output_dir: str, file_name: str, ext=".spacy"
) -> None:
...
def main():
    ...
    parser.add_argument(
        "-s", "--spacy-dir", help="Path to the folder containing .spacy files."
    )
    parser.add_argument(
        "-o",
        "--output-dir",
        help="Path to the folder where training, validation and the test .spacy files will be created.",
    )
    parser.add_argument(
        "-l",
        "--language-model",
        default="en_core_web_trf",
        help="Name of the language model used as a storage class for vocabulary.",
    )
    parser.add_argument(
        "-t",
        "--train-pct",
        default=0.7,
        type=float,
        help="Percentage of the total number of samples that will be used for the model training.",
    )
    parser.add_argument(
        "-v",
        "--val-pct",
        default=0.2,
        type=float,
        help="Percentage of the total number of samples that will be used for the model validation.",
    )
