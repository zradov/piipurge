...
def _augment_annots(
    docs_annots: List[Tuple[str, List]], max_augmented_examples: int
) -> List[Tuple[str, List]]:
...
def _oversample(
    ...
    sorted_annots = dict(
        sorted(annots_groupped_by_entity.items(), key=lambda i: len(i[1][1]))
    )
...
def _undersample(
    ...
    sorted_annots = dict(
        sorted(
            annots_groupped_by_entity.items(), key=lambda i: len(i[1][1]), reverse=True
        )
    )
...
def balance_examples(
    docs_annots: List[Tuple[str, List]], max_majority_minority_ratio: float = 1.5
) -> List[Tuple[str, List]]:
    ...
    grouped_single_entity_annots = dict(
        sorted(grouped_single_entity_annots.items(), key=lambda i: len(i[1][1]))
    )
    ...
    balanced_examples = (
        single_entities_annots.values() + multiple_entities_annots.values()
    )
