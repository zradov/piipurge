...
def _get_date_format(date_value: str, default_format_index: int = 6) -> str:
    ...
    logging.info(
        f"Falling back to the default format: {DATE_FORMATS_REGEX[default_format_index]}"
    )
...
def _remove_overlapping_loc_annots(
...
def _insert_labels_placeholders(
    text: str, annotations: List[Tuple[int, int, str]]
...
def _find_entity_positions(
    sentence: str, labels: Dict[str, List[Tuple[str, str]]]
) -> List[Tuple[int, int, str]]:
    """
    ...
    for entity_type in labels:
        pattern = re.compile(
            f"\\[{LABEL_TYPE_DESC_MAPPINGS[entity_type]}\\d+(?:[:])?([^\\]]+)?\\]",
            flags=re.IGNORECASE,
        )
        for idx, match in enumerate(pattern.finditer(sentence)):
            label_value_format = (
                "" if entity_type != "DATE" else labels[entity_type][idx][1]
            )
            positions.append(
                (match.start(), match.end(), entity_type, label_value_format)
            )

    return sorted(positions, key=lambda x: x[0])


def _process_address_components(
    address: str, start_pos: int
) -> List[Tuple[int, int, str]]:
    """
...
def _process_sentence(
    sentence: str, entity_positions: List[Tuple[int, int, str]], faker: Faker
...
def _insert_fake_entities(
    sentences: List[str], labels: Dict[str, List[str]]
    ...
    for sentence in sentences:
        ...
        processed_sentence, new_entities = _process_sentence(
            sentence, entity_positions, faker
        )
