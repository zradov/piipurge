...
def analyze_print(saved_images_info: List[SavedImageInfo]) -> Dict[str, ImageTextInfo]:
    ...
    for img_info in saved_images_info:
        ...
        images_text[img_info.img_path] = ImageTextInfo(image_size=image.size, text_lines=text_lines)
...
def analyze_handwriting(img_paths: List[str]) -> List[Tuple[str, bool]]:
    ...
    transform = _get_image_transformations(consts.HANDWRITTEN_SIGNATURES_MEAN, 
...
def _analyze_print_from_image(image, model, processor):
    ...
    pixel_values = processor(image, return_tensors="pt", input_data_format="channels_last").pixel_values
...
def _are_textboxes_adjacent(
        text_box: List[int], 
        text_boxes: List[List[int]]) -> bool:
    """
...
def _get_text_bboxes(image, 
    ...
    for idx, ocr_text in enumerate(ocr_data["text"]):
        if ocr_text.strip() != "" or include_empty_text:
            ...
            generated_texts, scores = _analyze_print_from_image(cropped_img, vision_model, image_processor)
            max_score_index = torch.argmax(scores).item()
            if scores[max_score_index].item() > score_threshold:
                best_text = None
                best_similarity_score = 0.0
                for gen_text in generated_texts:
                    text_similarity_score = get_text_similarity(gen_text, ocr_text).item()
...
def _get_text_lines(text_bboxes: Dict) -> List[Dict]:
    ...
    for i in range(len(text_bboxes) - 1):
        ...
        for j in range(i + 1, len(text_bboxes)):
            if _are_textboxes_adjacent(text_bboxes[j]["bbox"], [t["bbox"] for t in related_text_boxes]):
